<div class="bulid_info">
    ...
    <div class="detail_msg">
        <ul>
            ...
            <li>
                <span class="black w2">配送</span>
                @if(isset($data->productToShop->productToProvince->province)){{$data->productToShop->productToProvince->province}}@endif @if(isset($data->productToShop->productToCity->city)){{$data->productToShop->productToCity->city}}@endif
                &nbsp;至<em id="city"/>{{$freight->city}}</em>
                <label id="yundan">
                    快递<em id="yundan02">{{$freight->name}} - {{$freight->price}}</em>
                </label>
            </li>
            ...
            @endforeach
            <input type="hidden"  id="newprice" name="newprice" @if( (int)$data->preferential_price ) value="{{encode($data->preferential_price)}}" @else value="{{encode($data->price)}}" @endif/>
            <!---可配结束-->
            ...
            <li class="phone_umber">
                <a href="javascript:;" @if( Session::get('member') ) class="u-buy1 buy-button" data-url="{{url('goods/purchase')}}" @else @if( getConfig('core','guset_shop') ) class="u-buy1 buy-button"  data-url="{{url('/goods/dontlogin/purchase')}}"  @else class="u-buy1 keepNo"  @endif @endif rel="nofollow" >立即购买</a>
            ...
            {{ Form::close() }}
            <input type="hidden" id="freight" value="{{$data->freight}}"/>
            <input type="hidden" id="weight"  value="{{$data->weight}}"/>
