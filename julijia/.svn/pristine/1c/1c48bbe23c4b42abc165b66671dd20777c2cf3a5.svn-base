                ...
                <tr class="order-tr" name="1">
                    ...
                    <td>
                        <div class="goodsPicBox pull-left m-r-10">
                            <img src="{{ getImgSize( 'goods', $row->entity_id, isset($row->product->small_image)?$row->product->small_image:'' ) }}" class="goods-thumb">
                        </div>
                        <div class="ng-binding refund-message">
                            <div class="name">
                                <a href="/{{$row->entity_id}}.html" target="_blank" data-toggle="tooltip" data-placement="auto bottom" title="{{$row->item->product_name}}"
    ...
    $(function(){
        ...
        $("#replay").click(function(){
            ...
            $.post("/admin/comment/check",$("#replay_form").serialize(),function(data){
                var is_tong = $("#replay_form input:radio:checked").val();
                if(data.status == 1) {
