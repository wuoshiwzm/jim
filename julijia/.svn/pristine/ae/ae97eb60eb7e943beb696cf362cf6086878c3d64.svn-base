    ...
    <div id="pagination" class="pull-right">
        <form  class="searchfrm" action="{{Request::url()}}" method="get">
            ...
            <div class="pagination-goto">
                <input class="ipt form-control goto-input" type="text" id="setpage" name="page" value="{{$data->getCurrentPage()}}">
                <button class="btn btn-default goto-button" title="点击跳转到指定页面"  onclick="searchGet( this,{{$data->getLastPage()}} )">GO</button>
            </div>
        </form>
    </div>
</div>
