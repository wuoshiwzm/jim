        ...
        $result = DB::transaction(function( )
        {
           ...
           $info=  Source_Order_OrderInfo::where('order_sn',$input)->where('status','1')->first();
            ...
            $item['shipping_status'] =1;
             Source_Order_OrderItem::where('order_id',$info->id)->update($item);
            //写支付宝记录
            ...
            $Item = Source_Order_OrderItem::where('order_id',$info->id)->select('product_id','num','order_id')->get();
            foreach ( $Item as $i )
            {
                $ProductFlat = Source_Product_ProductFlat::where('entity_id',$i->product_id)->select('kc_qty','entity_id')->first();
                $ProductFlat->kc_qty = (int)($ProductFlat->kc_qty-$i->num);
                $ProductFlat->save();

                $Stock = Source_Product_ProductEntityStock::where('entity_id',$i->product_id)->select('stock','entity_id')->first();
