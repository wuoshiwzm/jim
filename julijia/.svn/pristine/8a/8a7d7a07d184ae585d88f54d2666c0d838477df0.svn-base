...
<div class="page">
    ...
    <div class="common-title">
        <div class="ftitle">
            ...
            <h5>
                (&nbsp;共
                <span data-total-record="true">{{$users->getTotal()}}</span>
                条记录&nbsp;)
            </h5>
        ...
        <table id="table_list" class="table table-hover">
            ...
            @if($users->getTotal() > 0 )
                    @foreach($users as $row)
                        <tr>
                        <td class="tcheck">
                            <input class="checkBox table-list-checkbox" value="{{$row->user_id}}" type="checkbox">
                        </td>
                        <td>
                            {{$row->user_id}}
                        </td>
            ...
            @if ($users->getTotal() > 0)
                <tr>
                    <td class="text-c w10">
                        <input class="table-list-checkbox-all check-all" title="全选/全不选" type="checkbox">
                    </td>
                    <td colspan="4">
                        <div class="pull-left">
                            <input class="btn btn-danger m-r-2 batch-offsale-goods batch-delete" value="删除" type="button">
                        </div>
                    </td>
