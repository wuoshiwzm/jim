                    ...
                    <tr class="order-item">
                        <td class="tcheck">
                            @if($row->shipping_status == 1)
                                <input class="checkBox table-list-checkbox" data-item="{{$row->id}}" type="checkbox" @if($row->refund()->count()>0) disabled="disabled" @endif>
                            @endif
                        </td>
                        <td class="item" width="30%">
                            <div class="pic-info">
                                ...
                                @else
                                    <a  class="goods-thumb" title=" {{$row->product_name}}" target="_blank">
                                        <img src="{{ getImgSize( 'goods', $row->product_id, '') }}" alt=" {{$row->product_name}}">
                                    </a>
                                @endif
                            ...
                            <div class="txt-info">
                                <div class="desc">
                                    <a class="goods-name" href="##" target="_blank" title=" {{$row->product_name}}">
                                        {{$row->product_name}}
                                    </a>
                                </div>
                            </div>
                        ...
                        <td class="" >
                            ...
                            @elseif($row->refund->count()>0)
                                <a href="/admin/refund/index" >退货退款中</a>
                            @endif
                        ...
                        <td class="handle" >
                          ...
                          <span class="text-c">
                            @if($row->shipping_status == 1 &&$row->refund->count()== 0)
