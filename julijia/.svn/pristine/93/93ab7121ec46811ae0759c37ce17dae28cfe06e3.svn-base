...
class IndexController extends CommonController
{
    ...
    public function getMain()
    {
        ...
        if(!Cache::has('admin_home')){
            ...
            for($i = 0; $i < 80; $i++){
                $index_date['time'][$i]=  date('Y-m-d', strtotime('-'.$i.' day'));
            }
    ...
    public function  getTubiao(){
        $res  = array();
        for($i = 0; $i < 80; $i++){
            $list = Source_User_UserInfo::whereRaw(DB::raw("date_format(created_at,'%Y-%m-%d') = '".date('Y-m-d', strtotime('-'.$i.' day'))."'"))
                ->count();
            // $res[$i][]= 'Date.UTC('.strftime(date('Y', strtotime('-'.$i.' day'))).','.strftime(date('m', strtotime('-'.$i.' day'))).','.strftime(date('d', strtotime('-'.$i.' day'))).')';
            $res[$i][]=  date('Y-m-d', strtotime('-'.$i.' day'));
            $res[$i][]=$list;
        }

       return  json_encode(array_reverse($res));
