...
function getImgSize( $directories, $id, $path, $w=null, $h= null  )
{
    ...
    {
        return Config::get('tools.imagePath').'config/product/'.getConfig('config_product','default_images');
    }
    if( $w == null || $h == null )
    {
 ...
 *
 */
function getConfig( $table=null, $name, $type=null )
{
    if(!Cache::has('config')){
        $res1 = DB::table('config_shipping');
        $res2 = DB::table('config_payment');
        $res3 = DB::table('config_product');
        $res = DB::table('config_core')->union($res1)->union($res3)->union($res2)->get();
        Cache::put('config',$res,5 );
    }
        $returndata =  Cache::get('config');
    foreach ($returndata as $key=>$val){

        if( $val->name == $name &&$val->name == 'website_logo' && $type == false )
        {
            return Config::get('tools.imagePath').'/config/core/'.$val->value;
        }
        if( $val->name == $name &&$val->name == 'admin_website_logo' && $type == false )
        {
            return Config::get('tools.imagePath').'/config/core/'.$val->value;
        }
