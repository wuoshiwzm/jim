...
{
    var pid = $(index).find("option:selected").val();
    if( pid )
    {
        $.post('/admin/product/goods/freight',{pid:pid},function (msg) {
            $("#info1").remove();
            $("#info2").remove();
            $("#freightinfo").append( msg );
        });
    }else
    {
        $("#info1").remove();
...
var pid = $("#freight").find("option:selected").val();
if( pid )
{
    var index = $("#freight");
    setFreight( index );
