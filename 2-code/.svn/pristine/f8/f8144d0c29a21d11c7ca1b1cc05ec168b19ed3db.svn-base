...
@section("content")
    <div class="page">
        ...
        <div class="search-term m-b-10">
            <form id="searchForm" method="get" action="{{url('/admin/deliver/index')}}">
                ...
                @foreach( $data as $row )
                    ...
                    @foreach($row->item as $key => $goods_item )
                        <tr class="order-item">
                            <td class="item">
                                <div class="pic-info">
                                    <a href="/{{$goods_item->product_id}}.html" class="goods-thumb" title="{{$goods_item->product_name}}" target="_blank">
                                        <img src="{{ getImgSize( 'goods', $goods_item->product_id, isset($goods_item->product->small_image)?$goods_item->product->small_image:'' ) }}" alt="{{$goods_item->product_name}}">
                                    </a>
                                </div>
                                <div class="txt-info">
                                    <div class="desc span02">
                                        <a href="/{{$goods_item->product_id}}.html" class="goods-name" target="_blank" >{{$goods_item->product_name}}</a>
                                    </div>
                                </div>
                            </td>
                            <!--单价-->
                            ...
                            <td class="contact contact01" sumrows="1" rowspan="1">
                                {{isset($goods_item->Supplier->supplier_name)?$goods_item->Supplier->supplier_name:''}}
                            </td>
                            <!--运输状态-->
                            <td class="trade-status" sumrows="1" rowspan="1">
                                @if($goods_item->shipping_status == 1)
                                    未发货
                                @elseif($goods_item->shipping_status == 2)
                                    已发货
                                    <br>
                                    <a href="{{url("/admin/order/ems?".http_build_query(["l_id" =>$goods_item->shipping_id,'item_id'=>$goods_item->id]))}}". class="yundan"><font>运单</font>{{$goods_item->shipping_id}}</a>
                                    <br>
                                @elseif($goods_item->shipping_status == 3)
                                    已收货
                                @endif
                            </td>
                            ...
                            <td class="handle" sumrows="1"  rowspan="{{$row->item()->count()}}" >
                                <div class="ng-binding">
                                        ...
                                        <span class="text-c">
                                            ...
                                            <a href="{{url("/admin/order/deliver?".http_build_query(["order_id"=>$row->id]))}}">发货</a>
                                        </span>
                                </div>
                            </td>
                            <!-- 共用end -->
