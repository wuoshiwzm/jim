...
<div class="page">
    ...
    <div class="subject subject_h4">
        <h4>
            支付金额<span class="action">：</span>￥{{$order->pay_amount}}
        </h4>
    </div>
    ...
    <div class="table-responsive deliver">
        <table class="table">
            ...
            <tr class="order-item">
                ...
                <td class="item" >
                    <div class="pic-info">
                        <a class="goods-thumb" title="{{$row->product_name}}" target="_blank">
                            <img src="{{ getImgSize( 'goods', $row->product_id, isset($row->product->small_image)?$row->product->small_image:'' ) }}" alt="查看商品详情">
                        </a>
                    </div>
                    <div class="txt-info">
                        <div class="desc span02">
                            <a href="/{{$row->product_id}}.html" class="goods-name" target="_blank" >{{$row->product_name}}</a>
                        </div>
                    </div>
                </td>
                <td class="price">
                ...
                <td class="discount postage">
                    <div class="ng-binding">
                             <span>
                            <input class="form-control valid disprice" type="text" placeholder="请输入调整值"
                ...
                @if ($key == 0)
                    <td class="postage" rowspan="{{$order->item()->count()}}" sumrows="1">
                        <div class="ng-binding">
                        <span>
                        <input class="form-control valid freight" type="text"   placeholder="请输入运费"   autocomplete="off" >
                        </span>
                        </div>
                    </td>
                @endif
