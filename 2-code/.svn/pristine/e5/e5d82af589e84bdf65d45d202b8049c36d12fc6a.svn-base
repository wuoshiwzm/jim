...
class AuctionBuyController extends \BaseController
{
    ...
    public function auctionBuyNow()
    {
        ...
        $uid = self::$user->id;
        $region = Source_User_UserInfoAdd::where(['user_id'=>$uid,'status'=>1])->pluck('district');
        $goodsData = AuctionBuy::buyNow( $data,$productInfo,$region);
        //产品信息
        ...
        $freight = Freight::DetailFreight( $productInfo->freight, $productInfo->weight, 0, (int)$data['num'], $type, $region  );
    ...
    public function cartOrder()
    {
        ...
        $itemId = decode($order);
        $region = Source_User_UserInfoAdd::where(['user_id'=>$uid,'status'=>1])->pluck('district');
        $goodsData = AuctionBuy::cartOrderData( $itemId, self::$user, $region);
        //产品信息
        $goods = $goodsData['goods'];
        //合计信息
        $totaled = $goodsData['totaled '];
    ...
    public function cartOrderSave()
    {
        ...
        $uid = self::$user->id;
        $region = Source_User_UserInfoAdd::where(['user_id'=>$uid,'status'=>1])->pluck('district');
        $goodsData = AuctionBuy::cartOrderData( decode($data['order']), self::$user, $region);
        if( $goodsData == false )
        {
