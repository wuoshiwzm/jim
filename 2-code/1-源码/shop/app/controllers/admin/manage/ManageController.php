...
class ManageController extends  CommonController
{
        ...
        try{
            ...
            if (count($userIds) != 0) {
                foreach($userIds as $userId) {
                    $user_admin = Source_User_AdminUser::find((int)$userId);
                    $user_admin->delete();
                }
            } else{
    ...
    public function  postSavepassword(){
        $data = Input::get();
        $res = Source_User_AdminUser::find(Session::get('admin_user')['user_id']);
        $resurt =array('status'=>0);
        if($res->password ==encode($data['oldpassword'])){
             $res->password = encode($data['newpassword']);
           if( $res->save()){
               $resurt['status'] =1;
           };
         }else{
    ...
    public function  postValidator()
    {
        ...
        try{
            $name = Input::get("param");
            //判断角色名称是否存在
            $role = Source_User_AdminUser::where("account",$name);
            if($role->count() > 0) {
                $result["status"] = "n";
                $result["info"] = "用户已经存在";
            }
