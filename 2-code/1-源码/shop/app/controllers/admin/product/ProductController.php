...
class ProductController extends CommonController
{
	...
	public function edit( $id )
	{
		...
		//运费
		$freight = Source_Shipping_ShippingTemplete::orderBy('is_xitong','desc')->get();
		$this->view('admin.product.edit',compact('data','shop','supplier','id','setID','flat','brand','freight'));
	}
	...
	public function update( $id )
	{
		...
		if ( $res )
		{
			...
			//清理缓存
			Event::fire('admin.operational.data',array(1));
			Event::fire('product.changeStatus',array($id));
	...
	public function add()
	{
		...
		//运费
		$freight = Source_Shipping_ShippingTemplete::orderBy('is_xitong','desc')->get();
		$this->view('admin.product.add',compact('data','categoryName','categoryID','supplier','shop','setID','brand','freight'));
	}
	...
	public function status()
	{
		...
		if( is_null( $result ) )
		{
		    Event::fire('product.changeStatus',array(Input::get('entity_id')));
			//清理缓存
			Event::fire('admin.operational.data',array(1));
			$obj = new stdClass();
			$obj->ststus = 0;
	...
	public function statusAll()
	{
		...
		$result = DB::transaction(function() use ( $status )
		{
			...
			{
				Event::fire('product.changeStatus',array($row));
				$aID = Source_Eav_Attrbute::where('name', 'status')->pluck('id');
	...
	{
		$pid = Input::get('pid');
		$freight = Source_Shipping_ShippingTemplete::find($pid);
		if( $freight->is_freeshipping )
		{
			$data = Source_Shipping_FreeShippingTiaoJian::where('shipping_id',$pid)->get();
			return View::make('admin.product.freightinfo',array('data'=>$data,'type'=>$freight));
		}else
		{
			$data = Source_Shipping_ShippingModel::where('pid',$pid)->get();
			return View::make('admin.product.freightinfo',array('data'=>$data,'type'=>$freight));
		}
	}
}
