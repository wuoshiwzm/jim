...
<div class="shengqing" style="display: none">
    ...
    <form id="form2" class="m-form">
        <ul>
            ...
            <li>
                <textarea id="content" class="textarea" name="content" placeholder="请输入审核意见"></textarea>
            </li>
            <li>
...
<script>
    $(function(){
        ...
        $("#confirm2").click(function(){
            if (!/^[\u4e00-\u9fa5_a-zA-Z]{2,10}$/.test($("#link").val())) {
                layer.msg("联系人姓名格式必须为中英文最大长度不超过10位",{icon:2,time:2000});
                return false;
            }else if( ! /^[0-9]{11}$/.test($("#link_phone").val())) {
                layer.msg("联系人电话必须为数字且最大不超多11位",{icon:2,time:2000});
                return false;
            }else if ($("#address").val() == "") {
                layer.msg("联系人地址不能为空");
                return false;
