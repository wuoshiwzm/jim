...
<div class="body">
    ...
    <form class="m-form" id="frm" accept-charset="UTF-8" action="" method="POST">
        ...
        <input type="hidden" value="" name="{{trim($i)}}" id="path">
    </form>
</div>
...
<script>
        ...
        $("#file_upload").uploadify({
            ...
            'onUploadSuccess': function (file, data, response) {
                if (data.length > 50 || data == false) {
                 ...
                 p =$("#path").val();
                 $("#path").val(p+data+';');
                 $(".img_a").append("<div><img src=/media/temp/"+data+"><span  onclick=delimg('"+$.trim(data)+"') >删除</span></div>");
    ...
    $("#subfrm").click(function () {
        if ($(".img_a").find('img').attr('src') != '/images/user.jpg') {
            ...
            var data = $("#path").attr('name');
            var imgInput = $("#path").val();
            parent.setPathUrl(imgInput, data);
