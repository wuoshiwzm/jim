...
@stop
@section('title','订单'.$order->order_sn)
...
//                                        echo "<dd><a target='_blank' href=".url(encode('test'))." class='btn'>评价</a></dd>";
                    ...
                    <div class="order-step02">
                        ...
                        <dl class="current">
                            ...
                            <dd class="bg02">{{$order->created_at}}</dd>
                        </dl>
                        <div class="order-step_bg order-step_bg_on">&nbsp;</div>
                        <dl @if($order->pay_status ==3) class="current"@endif>
                            <dt class="iconfont">&#xe610;</dt>
                            <dd class="bg"><font>付款成功</font></dd>
                            <dd class="bg02">
                                @if(($order->pay_status ==3) && $payTime)
                                    {{$payTime}}
                                @endif
                            </dd>
                        </dl>
                        <div class="order-step_bg order-step_bg_on">&nbsp;</div>
                        <dl @if($order->status >=4)class="current"@endif>
                            <dt class="iconfont">&#xe609;</dt>
                            <dd class="bg"><font>商品出库</font></dd>
                            <dd class="bg02">
                                @if(($order->status >=4) && $shipTime)
                                    {{$shipTime}}
                                @endif
                            </dd>
                        </dl>
                        <div class="order-step_bg">&nbsp;</div>
                        <dl @if($order->status >=5) class="current" @endif >
                            <dt class="iconfont">&#xe650;</dt>
                            <dd class="bg"><font>等待收货</font></dd>
                            <dd class="bg02">
                                @if(($order->status >=5) && $receiveTime)
                                    {{$receiveTime}}
                                @endif
                            </dd>
                        </dl>
                        <div class="order-step_bg">&nbsp;</div>
                        <dl @if($order->status >=7)  class="current"@endif >
                            <dt class="iconfont">&#xe659;</dt>
                            <dd class="bg"><font>完成</font></dd>
                            <dd class="bg02">
                                @if(($order->status >=7) && $accomplishTime)
                                    {{$accomplishTime}}
                                @endif
                            </dd>
                        </dl>
                    ...
                    @foreach($items as $item)
                        <div class="table_div table_div02">
                            <div class="order_wuliu">
                                <dl>
                                    <dt>
                                        <img src="{{ getImgSize( 'goods', $item->product_id, isset($item->product->small_image)?$item->product->small_image:'' ) }}">
                                    </dt>
                                    <dd><span>送货方式：</span><font>{{!empty($item->shipping_name)?$item->shipping_name:"暂无快递"}}</font></dd>
                                    <dd>
                                        <span>物流公司：</span><font>
                                            @if($item->shipper)
                                                {{isset($item->shipper->shipping_name)?$item->shipper->shipping_name:'无物流信息'}}
                                            @else
                                                无物流信息
                                            @endif
                                        </font>
                ...
                <div class="table_div">
                    ...
                    <div class="order_information">
                        <ul>
                            ...
                            <li><span>配送方式：</span><font>{{!empty($item->shipping_name)?$item->shipping_name:"暂无快递"}}</font></li>
                    ...
                    <div class="order_information">
                        <ul>
                            ...
                            <li><span>商品总额：</span><font>¥{{$order->cost_item}}</font></li>
                            <li><span>运费金额：</span><font>¥{{$order->shipping_amount}}</font></li>
                            <li><span>优惠金额：</span><font>¥{{$order->cost_freight}}</font>
                            <li><span>支付金额：</span><font>¥{{$order->pay_amount}}</font></li>
                            </li>
                        </ul>
                    </div>
                   {{-- <div class="order_information">
                                ...
                                <tr>
                                    <td class="padding_left">
                                        <dl>
                                            ...
                                            <dd>
                                                <a href="/{{$item->product_id}}.html" target="_blank">{{$item->product_name}}</a>
                                            </dd>
                                    ...
                                    <td>{{isset($item->product->sku)?$item->product->sku:''}}</td>
                                    <td>
                                        <font class="price">¥ {{$item->price}}</font>
                                    </td>
                                    <td>{{$item->num}}</td>
                                    <td>
                                        @if($item->shipping_status ==1 && $item->refund->count() ==0)
                                            未发货
                                        @elseif($item->shipping_status ==2&& $item->refund->count() ==0)
                                            已发货
                                        @elseif($item->shipping_status ==3&& $item->refund->count() ==0)
                                            已收货
                                        @elseif($item->refund->count() ==1)
                                            已退款退货
                                        @endif
                                    </td>
                                    @if($item->shipping_status ==1&&$order->pay_status ==3 &&$item->refund->count() ==0)
                                        <td class="operation ">
                                            <a href="{{url('member/refund/apply_refund/'.encode($order->id).'/'.encode($item->id))}}">退款</a>
                                        </td>
                                    @elseif($item->shipping_status ==3&& $order->pay_status ==3&&$item->refund->count() ==0)
                                        <td class="operation ">
                                            <a href="{{url('member/refund/apply_refund/'.encode($order->id).'/'.encode($item->id))}}">退货</a>
                                        </td>
                                    @elseif($item->shipping_status ==2 && !$item->refund()->count())
                                        <td class="operation ">
                                            <a href="javascript:void(0)" class="margin_top02" onclick="receive('{{encode($order->id)}}','{{encode($item->id)}}')">确认收货</a>
                                        </td>
                                    @elseif($order->status ==1)
                                        <td><a href="/member/orderitem/remove/{{encode($item->id)}}" class="margin_top02" >删除商品</a></td>
                                        @else
                                     <td></td>
                                    @endif
                        ...
                        <div class="jiesuan">
                            ...
                            <div class="jiesuan_right">
                                <ul>
                                    ...
                                    <li>
                                        <span>优惠金额：</span><font>&nbsp;¥&nbsp;{{$order->cost_freight}}</font>
                                    </li>
                                    <li>
                                        <span>运费：</span><font>¥&nbsp;{{$order->shipping_amount}}</font>
                                    </li>
                                    <li><span>商品总价：</span>
                                        <font>¥&nbsp;{{$order->total_amount}}</font>
                                    </li>
                                </ul>
                            </div>
                        </div>
