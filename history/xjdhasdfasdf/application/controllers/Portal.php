    ...
    function powermeter_history()
    {
        ...
        if(!empty($data_id) && count($dateRangeArr) == 2 )
        {
            ...
            if($export == "exporttoexcel")
            {
            	...
            	$PAGE_SIZE=2000;
            	$xls = new Excel_XML('UTF-8', false, '能耗分析模型');
            	$xls->addRow(array("日期","A相功率","B相功率","C相功率","合相功率","A相电压","B相电压","C相电压","A相电流","B相电流","C相电流", "合相电流","A相电能","B相电能","C相电能","合相电能"));
    ...
    function device_history ()
    {
    	...
    	if(!empty($data_id) && count($dateRangeArr) == 2)
    	{
    		...
    		if($export == "exporttoexcel")
    		{
    			...
    			if(in_array($model,array('fresh_air'))){    //网页未完成
    				$xls->addRow(array('采集时间','室内温度1','室内温度2','室内温度3','室内温度4','室内温度5','室内湿度1','室内湿度2','室内湿度3','室内湿度4','室内湿度5',
    			...
    			if(in_array($model,array('psm-6'))){    //网页未完成
    				$xls->addRow(array ('采集时间',"交流输入类型","输入交流过压保护值(V)","输入交流低压保护值(V)","配电输出总数","电源系统整流模块总数","系统控制方式","电池总数(组)","电池容量(Ah)","浮充电压(V)","均充电压(V)","均充时间间隔(天)","均充定时时间(小时)","充电系数","馈线电阻(mΩ)",
    						"电流充电限流值(A)","均浮充转换电流(A)","电池欠压报警值","电池欠压保护值","电池欠压是否自动保护","配电监控单元地址(00-99)"));
    			}

    			if(in_array($model,array('aeg-ms10se'))){ //网页未完成
    			...
    			if(in_array($model,array('datamate3000'))){  //网页未完成
    				$xls->addRow(array('采集时间','室内温度','室内湿度','室外温度','空调开关机状态','开机温度','关机湿度','温度设定点','温度偏差','湿度设定点','湿度偏差','开/关机状态','风机状态','制冷状态','加热状态','加湿状态','除湿状态','机组状态','机组属性','高压锁定状态','低压锁定状态','排气锁定状态','温度设定点','湿度设定点','高温告警点','低温告警点','高湿报警点','低湿报警点','高压报警',
    			...
    			if(in_array($model,array('liebert-ups'))){  //网页未完成
    				$xls->addRow(array("采集时间","数据ID",));
    			}

    			if(in_array($model,array('ug40'))){    //网页未完成
    			...
    			while(true)
    			{
    				...
    				foreach($data['powerDataList'] as $key => $powerDataObj)
    				{
    					...
    					if(in_array($model,array('battery_24','battery_32','battery24_voltage'))){
                            $n = count($powerDataObj->battery_voltage); $single = array(); $device_history = array();
                            for($i = 0; $i<$n; $i++){
                            ...
                            array_merge($device_history,$single);
                            $xls->addRow($device_history);
    					}
    					if(in_array($model,array('humid','temperature','water'))){
    						$xls->addRow(array(
...
//     		    	                }
    ...
    public function get_dv_threshold ()
    {
        ...
        echo json_encode(array("ret" => 0,"setting" => json_decode($dvObjs->setting)));
    }
        ...
        foreach(Constants::$devConfigList as $devConfig)
        {
        	...
        	if($devConfig[2] == "battery_32"){
        		...
        		foreach($dataList as $key=>$dataObj){
        			if(json_decode($dataObj->extra_para)->battery_index){
        				$obj->room_id = $dataObj->room_id;
        				$obj->data_id[json_decode($dataObj->extra_para)->battery_index] = $dataObj->data_id;
        			...
        			if($model == "enviroment")
        			{
            			...
            			//	$devDcList = $this->mp_xjdh->Get_DeviceDynamicConfig($dataObj->data_id);
            				switch($model)
            				{
                                ...
                                case "canatal":
            					case "battery_32":{
            						if( is_array($dataObj->data_id)){   //判断多组电池板采集同一组电池的data_id
            							$data['array'] = $array = array_values($dataObj->data_id);
            						}
            						$dataObj->html = $this->load->view ("portal/DevicePage/battery_32",$data, TRUE );
            						$dataObj->html1 = $this->load->view ("portal/standard_data", $data, TRUE);
            					}
            					case "camera":
            					case "freshair":
...
//     		$dbObj->where('city_code',$selCity);
   ...
   function edit_user_auth($uid=0){
   	...
   	function perforSetting ($substation_id)
   	{
   		...
   		array_push($data['bcList'], $bcObj);
   		$bcObj = new Breadcrumb();
   		$bcObj->title = '局站性能管理评估';
   		array_push($data['bcList'], $bcObj);
   		$bcObj = new Breadcrumb();
   		$bcObj->title = '添加局站性能管理规则';
   		$bcObj->isLast = true;
   		array_push($data['bcList'], $bcObj);
