...
//	}
    ...
    function Get_FaultList ($cityCode, $countyCode, $statusArr, $startDatetime, $endDatetime, $devModelArray, $level, $category, $ecType, $offset = 0, $limit = 20)
    {
    	$dbObj = $this->load->database('default', TRUE);
    	...
    	if($category == '0'){  //按次数统计
    		...
    		$dbObj->order_by('count(substation.name)', 'desc');
     		...
     		$dbObj->select('substation.name as subname, sum(alert_realtime.restore_datetime - alert_realtime.added_datetime) as sum');
     		$dbObj->group_by('substation.name');
     		$dbObj->order_by('sum(alert_realtime.restore_datetime - alert_realtime.added_datetime)', 'desc');	
    	}
    	$query = $dbObj->get('alert_realtime')->result();
