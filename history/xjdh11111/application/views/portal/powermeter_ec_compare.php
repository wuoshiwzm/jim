			...
			<div class="span12">
				<div class="content-widgets light-gray">
					...
					<div class="widget-container">
						...
						<button class="btn btn-success" type="button" id="btnNew">新增一个参与对比的组织</button>
					   <button class="btn btn-success" type="button" id="btnRemove">删除一个参与对比的组织</button>
					   <h4>提示：参与比较的组织维度以第一个为准，不符合的参与组织将被忽略</h4>
					   	</div>
						<div class="control-group department" id="departmentClone">
							...
							<label class="control-label" style="float: left;">业务维度</label>
								<div class="controls" style="margin-left: 20px; float: left;">
										<?php foreach(Defines::$gECType as $key=> $val){?>
										    <label style="float:left;width:100px;"><input type="radio" name="ecType"  <?php if($key == $ecType){?> checked="checked"  <?php }?> value="<?php echo $key; ?>" />&nbsp;&nbsp;&nbsp;&nbsp;<?php echo htmlentities($val,ENT_COMPAT,"UTF-8");?></label>	
										<?php } ?>
								</div>
							</div>
							<div class="control-group">
					...
					<div class="row-fluid">
					   <div class='span12'>
					        <h4><?php echo Defines::$gECType[$ecType]; ?>能耗</h4>
							<table class="table table-bordered responsive table-striped table-sortable">
							     <thead>
							         <tr>
    							         <th>时间</th>
    							         <?php foreach($labelArray as $labelValue){ ?>
    							         <th><?php echo $labelValue; ?></th>
    							         <?php } ?>
							     ...
							     <tbody>
							         ...
							         <tr>
							             <td><?php echo $date; ?></td>
							             <?php foreach($labelArray as $labelValue){ ?>
							             <td><?php echo $ec[$labelValue]; ?>度</td>			
							             <?php } ?>
							         </tr>
							         <?php } ?>
							     </tbody>
