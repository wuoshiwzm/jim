<div class="tab-widget tabbable tabs-left chat-widget">
	...
	<?php $index = 0;foreach ($batList as $batObj){?>
		<li <?php if($index++ == 0){?> class="active" <?php }?>><a  class="batterylist"  data_id='<?php echo htmlentities($batObj->data_id,ENT_COMPAT,"UTF-8");?>' 
				...
				<tr>
					...
					<tr>
						...
						<td class="hasThreshold"><button data_id='<?php echo htmlentities($batObj->data_id,ENT_COMPAT,"UTF-8");?>'
									field="group_voltage" class="btn btn-warning setThreshold">设置阈值</button></span></td>
					</tr>
					<tr class="current" style="display:none;">
						<td>整组电流</td>
						<td class="group_i"></td>
						<td class="update_datetime"><?php echo date('Y-m-d H:i:s');?></td>
						<td class="hasThreshold"><button data_id='<?php echo htmlentities($batObj->data_id,ENT_COMPAT,"UTF-8");?>'
					...
					<tr class="temp" style="display:none;" data='<?php echo htmlentities($batObj->data_id,ENT_COMPAT,"UTF-8");?>'>
						<td>整组温度</td>
						<td class="bat_temp"></td>
						<td class="update_datetime"><?php echo date('Y-m-d H:i:s');?></td>
						<td class="hasThreshold"><button data_id='<?php echo htmlentities($batObj->data_id,ENT_COMPAT,"UTF-8");?>'
									field="group_voltage" class="btn btn-warning setThreshold">设置阈值</button></span></td>
					</tr>
			</table>
			<?php $row = json_decode($batObj->extra_para)->amount; ?>
			<table
				class="table table-bordered table-striped responsive table-sortable"
