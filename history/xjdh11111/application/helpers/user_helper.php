...
class User
{
   ...
   function Get_UserLoginCount($txtName = false,$userRole = false,$userAgent = false,$selCity = false,$selCounty = false,$selSubstation = false,$datestart = false,$dateend = false,$cityCode = false){
       $ci = &get_instance();
       $dbObj = $ci->load->database('default', TRUE);
   ...
   function Get_UserLoginList($txtName = false,$userRole = false,$userAgent = false,$selCity = false,$selCounty = false,$selSubstation = false,$datestart = false,$dateend = false, $offset = 0, $size = 0,$cityCode = false){
       $ci = &get_instance();
       $dbObj = $ci->load->database('default', TRUE);
       $dbObj->join('user as u', 'l.user_id=u.id', 'left');
       if ($txtName != '')  $dbObj->like('u.full_name', $txtName);
