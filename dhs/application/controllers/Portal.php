...
//     		echo $success;
    ...
    public function substation_list($cityCode = '')
    {
        ...
        //die;
        //登录者  为 admin 或
        //$this->mp_xjdh  model 文件夹
        // function Search_Substation($name, $city_code='',
        // $substationIdList = array(), $size=0, $offset=0,
        // $gCounty = false)
        if (in_array($this->userObj->user_role, array("admin", "noc"))) {
            $data['substationList'] =
                $this->mp_xjdh
                    ->Search_Substation($keyWord, $cityCode,
                        null, null, null, $gCounty);
        } else {
            if ($cityCode != $this->userObj->city_code) {
                //没有权限
                redirect("/portal");
            }
            //在权限范围内，显示可以查看的局站   如果role为city_admin 可以查看所有城市
            if (in_array($this->userObj->user_role, array("city_admin"))) {
                //显示分公司所有局站
                $data['substationList'] =
                    $this->mp_xjdh->Search_Substation($keyWord, $cityCode,
                        $substationIdList, $size, $offset, $gCounty);
            } else {
                //根据用户授权表，显示可以查看的局站
                ...
                if (count($userPrivilegeObj)) {
                    ...
                    $data['substationList'] = $substationList =
                        $this->mp_xjdh->Search_Substation($keyWord, $cityCode,
                            $substationIdArray, $size, $offset, $gCounty);
                } else {
    ...
    {
        //$model 监控类型：监控设备，机房环境，蓄电池组，开关电源，视频监控 等
        $this->load->driver('cache');
        $data = array();
        $data['userObj'] = $this->userObj;
        $data['actTab'] = 'rt_data';
        ...
        //var_dump($data);die;
        $data['substationObj'] = $substationObj =
            $this->mp_xjdh->Get_Substation($roomObj->substation_id);
        if (!$this->_Check_User_Privilege($substationObj)) {
            redirect("/portal");
        }
                ...
                if (count($dataList)) {
                    ...
                    if ($devConfig[2] == $model) {
                        ...
                        if ($model == "enviroment") {
                            ...
                            $data['deviceContentBody'] =
                                $this->load->view('portal/DevicePage/enviroment',
        ...
        //处理header和body的显示 处理非operator用户
                            ...
                            foreach ($dataList as $dataObj) {
                                ...
                                switch ($model) {
                                    ...
                                    default:
                                        ...
                                        $dataObj->html = $this->load->view("portal/DevicePage/$model",
                                            array("dataObj" => $dataObj, 'ExtraPara' => $a->c, 'userObj' => $this->userObj), TRUE);
                                        break;
                                }
                            }
                            $data["dataList"] = $dataList;
