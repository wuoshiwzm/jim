					...
					<div class="widget-container"
						...
						<form class="form-horizontal">
							...
							<div class="control-group">
								...
								<label class="control-label"  id='group' style="float: left; display: none;">所属分组<span id='must' style="display: none;">
								    <font size=4 color=red>&nbsp;</font></span></label>
									<div class="controls" style="margin-left: 20px; float: left;">
						...
						<table id="dataTable"
							...
							<thead>
								<tr>
									...
									<th>设备类型</th>
									<th>物理端口号</th>
									<th width=200 style="word-break:break-all">逻辑参数</th>
									<th>是否激活</th>
									<th>操作</th>
								</tr>
							</thead>
							...
							<tr device_id='<?php echo htmlentities($devObj->id,ENT_COMPAT,"UTF-8"); ?>' dataId='<?php echo htmlentities($devObj->data_id,ENT_COMPAT,"UTF-8");?>'>
									...
									<td>
							         <?php
                                        ...
                                        else if (in_array($devObj->model, array('temperature','humid')))
                                             echo '模拟量AI-' . htmlentities($devObj->port,ENT_COMPAT,"UTF-8");
                                        else
                                        	 echo '串口UART-' . htmlentities($devObj->port,ENT_COMPAT,"UTF-8"); ?>
						           </td>
									<td width=200 style="word-break:break-all"><?php if($devObj->extra_para != "null") echo htmlentities($devObj->extra_para,ENT_COMPAT,"UTF-8"); ?></td>
										...
										<div class="btn-toolbar row-action">
											...
											<?php }?>
												<button class="btn btn-white" data-original-title="添加白名单"
													...
													<?php }?>
													<?php if(strpos($alertWhitelistObj,htmlentities($devObj->data_id,ENT_COMPAT,"UTF-8"))){?>
													<i class="icon-edit">移除白名单</i>
                                                    <?php }?>
													<?php if(!strpos($alertWhitelistObj,htmlentities($devObj->data_id,ENT_COMPAT,"UTF-8"))){?>
													<i class="icon-edit">添加白名单</i>
													<?php }?>
