<div class="tab-pane active" id='tab-data'>
    <div class="tab-widget tabbable tabs-left chat-widget">
        <ul class="nav nav-tabs" style='width:200px'>
            ...
            <?php if ($model == 'sps' || ($model == 'camera' && $groupList)) {
                $i = 1; ?>
                <?php $index = 0;
                foreach ($groupList as $groupObj) {
                    if ($model == 'sps') {
                        $group = $groupObj->dev_group;
                    } else {
                        $group = "视频监控";
                        $dataid = $groupObj->data_id;
                    }
                    ?>
                    <li class="grouplist" dev_group="<?php echo 'group' . $i; ?>">
                        <a href="##" group='<?php echo $group; ?>' class='group'><?php echo $group; ?>  </a>
                        <ul class="spslist" id='ul-<?php echo 'group' . $i;
                ...
                foreach ($dataList as $dataObj) { ?>
                    ...
                    <?php } ?>>
                        <?php if ($model == 'smd_device') { ?>
                            <a href="#device-<?php echo $dataObj->device_no; ?>"
                               class="group"> <?php echo $dataObj->name; ?> </a>
                        <?php } else if ($model == 'battery_32') { ?>

                            <?php if (is_int($dataObj->extra_para)) { ?>
                                <a href="#device-<?php echo $dataObj->data_id; ?>"
                                   class="group"> <?php echo $dataObj->dev_group; ?> </a>
                            <?php } else { ?>
                                <a href="#device-<?php echo $dataObj->data_id; ?>"
                                   class="group"> <?php echo $dataObj->name; ?> </a>
                            <?php } ?>
            ...
            foreach ($dataList as $dataObj) { ?>
                <?php if ($model == 'smd_device') { ?>
                    <div id='device-<?php echo $dataObj->device_no; ?>'
                        ...
                        <div id="standard-<?php echo $dataObj->device_no; ?>"
                             style="display: none;"><?php echo $dataObj->html1; ?></div>
