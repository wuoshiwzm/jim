...
//    }
    ...
    function GetApplyInfo($type = null, $locID = null)
    {
        ...
        if ($type == 1) {
            ...
            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['devicesInfoList' => $ques]);
            echo json_encode($jsonRet);
            return;
        }
    }
    ...
    public function getEditSubs($userID = null, $type = null, $locationType = null)
    {
        ...
        if (($type == 1) && ($locationType == 1)) {
            ...
            $dbObj->where('check_apply.user_id', $userID);
        ...
        //设备 - 获取机房
        if (($type == 2) && ($locationType == 2)) {

            $dbObj->from('check_device');
            $dbObj->join('room', 'check_device.room_id = room.id');
            $dbObj->where('check_device.substation_id', $userID);
    ...
    public function getEditQuestions($type = null, $id = null)
    {
        ...
        if ($type == 1) {
            ...
            $dbObj->where('substation_id', $id);
            $dbObj->select('content');
            $res = $dbObj->get('check_apply')->row()->content;
            $contents = json_decode($res);
            foreach ($contents as $k => $con) {
                $ans['question_id'] = $k;
                $ans['question'] = $this->getQuestionContent($k);
                $ans['content'] = $con;
                $answer[] = $ans;
            }
        ...
        if ($type == 2) {
            ...
            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['devContentList' => $devs]);
            echo json_encode($jsonRet);
            return;
        }
    }
     ...
     * 获取问题内容
     */
    private function getQuestionContent($questionID)
    {
        $dbObj = $this->load->database('default', TRUE);
        $content = $dbObj->where('id', $questionID)->get('check_question')->row()->content;
        return $content;
     ...
     * 获取设备问题内容
     */
    private function getDevContent($devID)
    {
        $dbObj = $this->load->database('default', TRUE);
    ...
    /**
     * @param $type 1：城市  2：局站 3:机房 4:设备
    ...
    public function getLocations($type = null, $id = null)
    {
        ...
        if ($type == 1) {
            ...
            $dbObj->distinct('city_code');
            $citys = $dbObj->get('substation')->result();

            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['cityList' => $citys]);
            echo json_encode($jsonRet);
            return;
        } //获取局站
        elseif ($type == 2) {
            ...
            $substations = $dbObj->get('substation')->result();

            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['subList' => $substations]);
            echo json_encode($jsonRet);
            return;
        }//获取机房
        elseif ($type == 3) {
            ...
            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['roomList' => $rooms]);
            echo json_encode($jsonRet);
            return;
        ...
        elseif ($type == 4) {
            ...
            $jsonRet['ret'] = 0;
            $jsonRet['data'] = json_encode(['deviceList' => $devices]);
            echo json_encode($jsonRet);
            return;
        }
        $jsonRet['ret'] = 1;
        $jsonRet['data'] = json_encode('数据错误');
        echo json_encode($jsonRet);
        return;
    }
     ...
     * 写入故障内容
     */
    public function uploadMemo($userID = null, $cityID = null,
                               $subID = null, $roomID = null, $deviceID = null, $content = null)
    {
        //存储文件
        $file_path = "./public/portal/Check_image/";
        $fileName = $_FILES['uploadImg']['name'];
        ...
        $subID = $this->input->post('subID') ? $this->input->post('subID') : null;
