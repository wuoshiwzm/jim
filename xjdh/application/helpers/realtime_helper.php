...
//                    $dataObj->model . '.js"></script>';
    ...
    static function _GetPMBusPowerRC($model, $memData, $p42_46_number, $p42_46_size, $p41_41_number, $p41_43_number, $p41_44_number)
    {
        ...
        if (strlen($memData) >= $rcLen) {
            ...
            $v = unpack("I", substr($memData, 0, 4));
            $dataObj->data_id = $v[1];
            $v = unpack('f*', substr($memData, 4, 7 * 4));
            $dataObj->vu = number_format($v[1], 2);
            $dataObj->vl = number_format($v[2], 2);
            $dataObj->iu = number_format($v[3], 2);
            $dataObj->fu = number_format($v[4], 2);
            $dataObj->fl = number_format($v[5], 2);
            $dataObj->out_v_high = number_format($v[6], 2);
    ...
    static function Get_Dk04RtData($data_id)
    {
        ...
        if (strlen($memData) >= 37) {
            ...
            $dk04Obj->eeVhi = $v[4];
            $dk04Obj->eeVlo = $v[5];
            $dk04Obj->eeBatTemp = $v[6];
            $dk04Obj->eeEQper = $v[7];
            $dk04Obj->eeEQdur = $v[8];
            $dk04Obj->eeEQmanDur = $v[9];
            $dk04Obj->eeEQtrip = $v[10];
            $dk04Obj->eeEQv = $v[11];
            $dk04Obj->eeLVDS = $v[12];
            $dk04Obj->eeBTC = $v[13];
    ...
    static function Get_Dk04cRtData($data_id)
    {
        ...
        if (strlen($memData) >= 1094) {
            ...
            $dk04Obj->ac_f_lo_alarm = $v[5];
            $dk04Obj->ac_i_fsd = $v[6];
            $dk04Obj->amb_temp_hi = $v[7];
            $dk04Obj->bat_temp_hi = $v[8];
            $dk04Obj->v_hi_alarm = $v[9];
            $dk04Obj->v_lo_alarm = $v[10];
            $dk04Obj->num_smrs = $v[11];
            $dk04Obj->smr_type = $v[12];
            $dk04Obj->fsd_current = $v[13];
            $dk04Obj->modem_enable = $v[14];
            $dk04Obj->emerg_call = $v[15];
            $dk04Obj->dialy_call = $v[16];
            ...
            $dk04Obj->i_bat_limit2 = $v[4];
            $dk04Obj->i_bat_limit3 = $v[5];
            $dk04Obj->deep_discharge = $v[6];
            $dk04Obj->float_volts = $v[7];
            $dk04Obj->eq_volts = $v[8];
            $dk04Obj->drop_volts = $v[9];
            $dk04Obj->bat_dis_alm = $v[10];
            $dk04Obj->v_start_eq = $v[11];
            $dk04Obj->eq_trip_v = $v[12];
            $dk04Obj->q_start_eq = $v[13];
            $dk04Obj->eq_trip_q = $v[14];
            $dk04Obj->eq_end_a = $v[15];
            $dk04Obj->eq_duration = $v[16];
            $dk04Obj->eq_period = $v[17];
            $dk04Obj->discharge_i_diff = $v[18];
            $dk04Obj->lvds = $v[19];
            ...
            if ($dk04Obj->smr_type == 0) {
                ...
                for ($i = 0; $i < $dk04Obj->num_smrs; $i++) {
                    ...
                    $channelObj->NumSMRs = $v[2];
                    $channelObj->FloatV = $v[3];
                    $channelObj->EQVolts = $v[4];
                    $channelObj->Vhi = $v[5];
                    $channelObj->V1o = $v[6];
                    $channelObj->HVSDtrip = $v[7];
                    $v = unpack("C*", substr($channelData, 14, 3));
                    $channelObj->I1im = $v[1];
                    $channelObj->AdjV = $v[2];
                ...
                for ($i = 0; $i < $dk04Obj->num_smrs; $i++) {
                    ...
                    $channelObj->AdjV = $v[2];
                    $channelObj->SecEnable = $v[3];
                    $v = unpack("C*", substr($channelData, 17, 6));
                    $channelObj->ver = $v;
                    $dk04Obj->param_channel[] = $channelObj;
                }
            }
    ...
    static function Get_Des7310RtData($data_id)
    {
         ...
         *   uint64_t data_id;
         *   GeneratingSetStatusInformation GSSIData;
         *   BasicInstrumentation BIData;
        ...
        if (strlen($memData) >= 99) {
             ...
             * unsigned short    BatteryChargerActiveCellCount;                    //电池充电活跃电池单元数量
             * unsigned short    BatteryChemistryID;                                //电池化学ID
             * unsigned short    ChargingStages;                                        //充电状态
             * unsigned short    ActiveCellCount;                                    //活跃电池单元数量
             */
            $v = unpack('S*', substr($memData, 4, 5 * 2));
            $DesObj->BatteryChargerMode = $v[1];//电池充电模式
            $DesObj->BatteryChargerActiveCellCount = $v[2];//电池充电活跃电池单元数量
            $DesObj->BatteryChemistryID = $v[3];//电池化学ID
            $DesObj->ChargingStages = $v[4];//充电状态
            $DesObj->ActiveCellCount = $v[5];//活跃电池单元数量
            ...
            if ($cacheValue) {
                ...
                if (!$memData) {
                    ...
                    if ($memData == false) {
                        ...
                        $dataObj->last_update = "无最后更新时间";
                    } else if ((time() - intval($memData)) > 600) {
                        //超时5分钟
                        $dataObj->status = false;
                        $dataObj->last_update = date("Y-m-d H:i:s", intval($memData));
                    } else {
                        $dataObj->status = true;
                        $dataObj->last_update = date("Y-m-d H:i:s", intval($memData));
                    }
    ...
    static function Get_Pmac600bRtData($dataIdStr)
    {
        $CI = &get_instance();
        $CI->load->driver('cache');
        $rtDataList = array();
        ...
        if (strlen($dataIdStr) > 0)
            $idArr = explode(',', $dataIdStr);
        foreach ($idArr as $key => $val) {
            $rtDataObj = new stdClass();
            $rtDataObj->data_id = $val;
            $memData = $CI->cache->get($val);
            if (strlen($memData) == 123) {
                $v = unpack('f*', substr($memData, 4, 112));
                $rtDataObj->ua = number_format($v[1], 3);
                $rtDataObj->ub = number_format($v[2], 3);
                $rtDataObj->uc = number_format($v[3], 3);
                $rtDataObj->ia = number_format($v[4], 3);
                $rtDataObj->ib = number_format($v[5], 3);
                $rtDataObj->ic = number_format($v[6], 3);
                $rtDataObj->psum = number_format($v[7], 3);
                $rtDataObj->qsum = number_format($v[8], 3);
                $rtDataObj->pf_sum = number_format($v[9], 3);
                $rtDataObj->pa = number_format($v[10], 3);
                $rtDataObj->pb = number_format($v[11], 3);
                $rtDataObj->pc = number_format($v[12], 3);
                $rtDataObj->qa = number_format($v[13], 3);
                $rtDataObj->qb = number_format($v[14], 3);
                $rtDataObj->qc = number_format($v[15], 3);
                $rtDataObj->cosfi_a = number_format($v[16], 3);
                $rtDataObj->cosfi_b = number_format($v[17], 3);
                $rtDataObj->cosfi_c = number_format($v[18], 3);
                $rtDataObj->freq = number_format($v[19], 3);
                $rtDataObj->ep_sum = number_format($v[20], 3);
                $rtDataObj->eq_sum = number_format($v[21], 3);
                $rtDataObj->ep_imp = number_format($v[22], 3);
                $rtDataObj->ep_exp = number_format($v[23], 3);
                $rtDataObj->eq_imp = number_format($v[24], 3);
                $rtDataObj->eq_exp = number_format($v[25], 3);
            }
            $rtDataObj->update_datetime = date('Y-m-d H:i:s');
            $rtDataObj->dynamic_config = $CI->cache->get($val . '_dc');
            array_push($rtDataList, $rtDataObj);
        }
        return $rtDataList;
    }

    static function Get_Pmac600aRtData($dataIdStr)
    {
        $CI = &get_instance();
        $CI->load->driver('cache');
        $rtDataList = array();
        ...
        foreach ($idArr as $key => $val) {
            ...
            $rtDataObj->data_id = $val;
            $memData = $CI->cache->get($val);
            if (strlen($memData) == 51) {
                $v = unpack('f*', substr($memData, 4, 40));
                $rtDataObj->current = number_format($v[1], 3);
                $rtDataObj->voltage = number_format($v[2], 3);
                $rtDataObj->ap = number_format($v[3], 3);
                $rtDataObj->rp = number_format($v[4], 3);
                $rtDataObj->pf = number_format($v[5], 3);
                $rtDataObj->freq = number_format($v[6], 3);
                $rtDataObj->sc = number_format($v[7], 3);
                $rtDataObj->rc = number_format($v[8], 3);
                $rtDataObj->ep = number_format($v[9], 3);
                $rtDataObj->eq = number_format($v[10], 3);
            }
            $rtDataObj->update_datetime = date('Y-m-d H:i:s');
            $rtDataObj->dynamic_config = $CI->cache->get($val . '_dc');
            array_push($rtDataList, $rtDataObj);
    ...
    static function Get_AiDiRtData($dataIdStr)
    {
        ...
        if (strlen($dataIdStr) > 0) {
            ...
            foreach ($roomDevList as $roomDevObj) {
                ...
                $memData = $CI->cache->get($roomDevObj->data_id . ":time");
                if ($memData == false) {
                    //没有时间的
                    $roomDevObj->status = false;
                    $roomDevObj->last_update = "无最后更新时间";
                } else if ((time() - intval($memData)) > 600) {
                    //超时5分钟
                    $roomDevObj->status = false;
                    $roomDevObj->last_update = date("Y-m-d H:i:s", intval($memData));
                } else {
                    $roomDevObj->status = true;
                    $roomDevObj->last_update = date("Y-m-d H:i:s", intval($memData));
                }
